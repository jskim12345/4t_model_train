...
if not os.path.exists(dkt_graph_path):
    dkt_graph_path = None
concept_num, graph, train_loader, valid_loader, test_loader, skill_mapping = load_dataset(dataset_path, args.batch_size, args.graph_type, dkt_graph_path=dkt_graph_path,
...
def test():
    ...
    with torch.no_grad():
        for batch_idx, (features, questions, answers, user_ids) in enumerate(test_loader):
            ...
            # # 00
            # print(f"Batch {batch_idx} - User IDs: {user_ids}")
            ...
            # # 예측 결과 출력 및 저장
            # print("Predicted Results:", pred_res)
            # all_pred_res.append(pred_res.cpu().numpy())  # GPU 사용 시 CPU로 변환
            # all_true_answers.append(answers.cpu().numpy())
    ...
    # # 모든 배치의 예측 결과를 numpy 배열로 변환
    # all_pred_res = np.concatenate(all_pred_res, axis=0)
    # all_true_answers = np.concatenate(all_true_answers, axis=0)
    # # 00

    # # 전체 결과 확인
    # for user_idx, user_id in enumerate(np.concatenate(all_user_ids)):
    #     print(f"User {user_id}:")
    #     print("Predicted:", all_pred_res[user_idx])
    #     print("Actual:", all_true_answers[user_idx])

    # import pandas as pd

    # # 예측 결과와 실제 정답을 데이터프레임으로 저장
    # results_df = pd.DataFrame({
    #     'user_id': np.concatenate(all_user_ids),
    #     'predicted': [pred.tolist() for pred in all_pred_res],
    #     'actual': [true.tolist() for true in all_true_answers]
    # })

    # # CSV 파일로 저장
    # results_df.to_csv('best_epoch_prediction_results.csv', index=False)
...
# 그래프, 그래프 모델 및 concept_num 저장
with open(f'{save_dir}/saved_graph_model_and_concept_num.pkl', 'wb') as f:
    pickle.dump((graph, graph_model, concept_num), f)  # 세 개를 튜플로 저장
test()
if log is not None:
    print(save_dir)
    log.close()