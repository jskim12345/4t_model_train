...
s3_bucket_name = "jstest-4t"                      
af_file_key = "raw/af_merged_df.csv"              # S3에 저장된 파일 경로
bf_file_key = "raw/bf_merged_df.csv"              # S3에 저장된 파일 경로
MODEL_PREFIX = "model/"
af_model_key = "model/af_deepfm_nn.keras"
bf_model_key = "model/bf_deepfm_nn.keras"
...
# S3에서 데이터 로드
af_merged_df = load_csv_from_s3(s3_bucket_name, af_file_key)
bf_merged_df = load_csv_from_s3(s3_bucket_name, bf_file_key)

af_merged_df = af_merged_df.dropna()
...
# 입력 피처와 타겟 분리
af_X = af_merged_df.drop('af_skill', axis=1)
af_y = af_merged_df['af_skill']
...
af_y_encoded = mlb.fit_transform(af_y.str.split('|'))
object_key = f"{MODEL_PREFIX}af_mlb.pkl"
...
# joblib.dump(mlb, 'af_mlb.pkl')


# 데이터 나누기 (훈련:검증:테스트 = 70:15:15)
af_X_train, af_X_temp, af_y_train, af_y_temp = train_test_split(af_X, af_y_encoded, test_size=0.3, random_state=42)
af_X_val, af_X_test, af_y_val, af_y_test = train_test_split(af_X_temp, af_y_temp, test_size=0.5, random_state=42)

af_num_classes = af_y_encoded.shape[1]  # 클래스 수
af_model = DeepFM(feature_size, embedding_size, af_num_classes)
af_model.compile(optimizer='adam', 
...
with mlflow.start_run():
    ...
    for epoch in range(epochs):
        ...
        history = af_model.fit(
            af_X_train, af_y_train,
            validation_data=(af_X_val, af_y_val),
    ...
    with tempfile.NamedTemporaryFile(suffix=".keras") as tmp:
        tf.keras.models.save_model(af_model, tmp.name)
        tmp.seek(0)

        # S3에 업로드
        # s3_client.upload_file(tmp.name, s3_bucket_name, af_model_key)
        # print("모델이 S3에 성공적으로 저장되었습니다:", af_model_key)
...
# history = af_model.fit(af_X_train, af_y_train, 
#                     validation_data=(af_X_val, af_y_val), 
#                     epochs=2, 
#                     batch_size=32)
...
# 모델 저장
# af_model.save('af_deepfm_nn.keras')
# 메모리 버퍼에 모델 저장
# model_buffer = io.BytesIO()
# tf.keras.models.save_model(af_model, model_buffer)  # 확장자로 .keras 사용, save_format 필요 없음   
# model_buffer.seek(0)  # 버퍼의 시작 위치로 이동

# # S3에 모델 업로드
# s3_client.upload_fileobj(model_buffer, s3_bucket_name, af_model_key)

with tempfile.NamedTemporaryFile(suffix=".keras") as tmp:
    tf.keras.models.save_model(af_model, tmp.name)  # .keras 파일로 임시 파일에 저장
    tmp.seek(0)  # 파일 포인터를 처음으로 이동
...
history = bf_model.fit(bf_X_train, bf_y_train, 
                    validation_data=(bf_X_val, bf_y_val), 
                    epochs=2, 
                    batch_size=32)

# 모델 저장
# bf_model.save('bf_deepfm_nn.keras')
# 메모리 버퍼에 모델 저장
# model_buffer = io.BytesIO()
# tf.keras.models.save_model(af_model, model_buffer)  # 확장자로 .keras 사용, save_format 필요 없음
# model_buffer.seek(0)  # 버퍼의 시작 위치로 이동
...
# # S3에 모델 업로드
# s3_client.upload_fileobj(model_buffer, s3_bucket_name, bf_model_key)

with tempfile.NamedTemporaryFile(suffix=".keras") as tmp:
    tf.keras.models.save_model(bf_model, tmp.name)  # .keras 파일로 임시 파일에 저장
    tmp.seek(0)  # 파일 포인터를 처음으로 이동
